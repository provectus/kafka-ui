...
public class ConsumingService {
	...
	public Mono<Map<TopicPartition, Long>> offsetsForDeletion(KafkaCluster cluster, String topicName, List<Integer> partitionsToInclude) {
	 ...
	 */
	private static Map<TopicPartition, Long> significantOffsets(Consumer<?, ?> consumer, String topicName,
																Collection<Integer> partitionsToInclude) {
	...
	@RequiredArgsConstructor
	static class RecordEmitter implements java.util.function.Consumer<FluxSink<ConsumerRecord<Bytes, Bytes>>> {
	...
	static class OffsetsSeek {
		...
		private List<TopicPartition> getRequestedPartitions(Consumer<Bytes, Bytes> consumer) {
			...
			return consumer.partitionsFor(topic).stream()
					.filter(p -> partitionPositions.isEmpty() || partitionPositions.containsKey(p.partition()))
