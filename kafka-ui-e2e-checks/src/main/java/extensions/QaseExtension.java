...
import utils.qaseIO.MethodNameUtils;
import utils.qaseIO.TestCaseGenerator;
...
/*    static {
        ...
        if (testMethod != null) {
            ...
            try {
                resultsApi.createResult(getConfig().projectCode(),
                        getConfig().runId(),
                        resultCreate);
                logger.info("Results added to test run with Id = " + getConfig().runId());
    ...
    private ResultCreate getResultItem(TestExecutionResult testExecutionResult, Duration timeSpent, Method testMethod) {
        ...
        TestCaseGenerator.createTestCaseIfNotExist(testMethod);
        ...
        for (Map.Entry<Long, String> map : cases.entrySet()) {
            if (map.getValue().matches(testCaseTitle)) {
                caseId = map.getKey();
                logger.info("Test case '" + MethodNameUtils.formatTestCaseTitle(testMethod.getName()) + "' with id = " + caseId +
